...
public abstract class Level implements Bundlable {
	...
	public void restoreFromBundle( Bundle bundle ) {
		...
		//saves from before v1.1.5 are not supported
		if (version < ShatteredPixelDungeon.v1_1_5){
	...
	public void updateFieldOfView( Char c, boolean[] fieldOfView ) {
		...
		if (sighted) {
			boolean[] blocking;
			if (c instanceof AbyssalNightmare || Dungeon.isChallenged(Conducts.Conduct.ALLSIGHT)){
        ...
        if (c.isAlive() && c == Dungeon.hero) {
			...
			for (Mob m : mobs){
				if (m instanceof WandOfWarding.Ward || m instanceof WandOfRegrowth.Lotus || m instanceof StationaryMinion){
