...
public class PoisonDartTrap extends Trap {
	...
	protected int poisonAmount(){
		return 8 + Math.round(2*Dungeon.depth / 3f* (5f / Dungeon.chapterSize()));
	}
	...
	public void activate() {
		...
		if (target != null) {
			...
			if (Dungeon.level.heroFOV[pos] || Dungeon.level.heroFOV[target.pos]) {
					...
					protected boolean act() {
						...
						((MissileSprite) ShatteredPixelDungeon.scene().recycle(MissileSprite.class)).
							reset(pos, finalTarget.sprite, new PoisonDart(), new Callback() {
								@Override
								public void call() {
									int dmg = Random.NormalIntRange(4, 8) - finalTarget.actualDrRoll();
				...
				finalTarget.damage(Random.NormalIntRange(4, 8) - finalTarget.actualDrRoll(), trap);
				Buff.affect( finalTarget, Poison.class ).set( poisonAmount() );
			}
		}
	}
