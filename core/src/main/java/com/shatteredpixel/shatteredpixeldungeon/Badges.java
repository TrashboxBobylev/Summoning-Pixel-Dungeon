...
public class Badges {
	...
	public enum Badge {
		...
		BOSS_SLAIN_3_SNIPER,
		BOSS_SLAIN_3_WARDEN,
        BOSS_SLAIN_3_SOUL_REAVER,
        BOSS_SLAIN_3_OCCULTIST,
	...
	static{
		...
		//v1.2.5 (These were removed and re-added internally as new unlock reqs were added)
	...
	public static void validateGoldCollected() {
		...
		if (!local.contains( Badge.GOLD_COLLECTED_1 ) && Statistics.goldCollected >= 3000) {
			if (badge != null) unlock(badge);
			badge = Badge.GOLD_COLLECTED_1;
			local.add( badge );
		}
		if (!local.contains( Badge.GOLD_COLLECTED_2 ) && Statistics.goldCollected >= 8000) {
			if (badge != null) unlock(badge);
			badge = Badge.GOLD_COLLECTED_2;
			local.add( badge );
		}
		if (!local.contains( Badge.GOLD_COLLECTED_3 ) && Statistics.goldCollected >= 20000) {
			if (badge != null) unlock(badge);
			badge = Badge.GOLD_COLLECTED_3;
			local.add( badge );
		}
		if (!local.contains( Badge.GOLD_COLLECTED_4 ) && Statistics.goldCollected >= 40000) {
			if (badge != null) unlock(badge);
			badge = Badge.GOLD_COLLECTED_4;
			local.add( badge );
		}
		if (!local.contains( Badge.GOLD_COLLECTED_5 ) && Statistics.goldCollected >= 66000) {
			if (badge != null) unlock(badge);
	...
	static {
		...
		thirdBossSubclassBadges.put(HeroSubClass.OCCULTIST, Badge.BOSS_SLAIN_3_OCCULTIST);
		thirdBossSubclassBadges.put(HeroSubClass.SOUL_REAVER, Badge.BOSS_SLAIN_3_SOUL_REAVER);
	}
...
//	}
	...
	//TODO will need to merge Summoning's and Shattered's scores together before implementing them
...
//	}
	...
	private static void displayBadge( Badge badge ) {
		...
		if (isUnlocked( badge )) {
			...
			if (!badge.meta) {
				GLog.highlight( Messages.get(Badges.class, "endorsed", badge.title()) );
				GLog.newLine();
			}
			...
			GLog.highlight(Messages.get(Badges.class, "new", badge.title() + " (" + badge.desc() + ")") );
			GLog.newLine();
