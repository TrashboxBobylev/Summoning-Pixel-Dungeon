				...
				protected void onSelect(int index) {
					if (index == 0){
						...
						if (tracker.depth != Dungeon.depth){
							GLog.warning( Messages.get(WarpBeacon.class, "depths") );
							return;
						}
						...
						if (charge < chargeNeeded){
							GLog.warning( Messages.get(ClassArmor.class, "low_charge") );
							return;
						}
...
//							if (timeBubble != null) timeBubble.disarmPressedTraps();
			...
			if (Dungeon.level.distance(hero.pos, target) > 8){
				GLog.warning( Messages.get(WarpBeacon.class, "too_far") );
				return;
			}
			...
			if (Dungeon.level.pit[target] ||
				...
				GLog.warning( Messages.get(WarpBeacon.class, "invalid_beacon") );
				return;
			}
