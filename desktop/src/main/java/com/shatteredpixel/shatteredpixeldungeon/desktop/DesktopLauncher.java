	...
	public static void main (String[] args) {
				...
				if (exceptionMsg.contains("Couldn't create window")){
					...
					TinyFileDialogs.tinyfd_messageBox(title + " Has Crashed!",
							title + " has run into an error it can't recover from and has crashed, sorry about that!\n\n" +
									"If you could, please email this error message to the developer (trashbox.bobylev@gmail.com):\n\n" +
		...
		if (SharedLibraryLoader.isWindows) {
			if (System.getProperties().getProperty("os.name").equals("Windows XP")) {
				basePath = "Application Data/.shatteredpixel/Summoning Pixel Dungeon/";
			} else {
				basePath = "AppData/Roaming/.shatteredpixel/Summoning Pixel Dungeon/";
			}
		} else if (SharedLibraryLoader.isMac) {
			basePath = "Library/Application Support/Summonin Pixel Dungeon/";
		} else if (SharedLibraryLoader.isLinux) {
			basePath = ".shatteredpixel/summoning-pixel-dungeon/";
		}
		...
		config.setWindowIcon("icons/icon_512x512.png");
